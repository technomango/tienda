...
class ProductController extends Controller
{
        ...
        if(!empty($products))
        {
            foreach ($products as $key=>$product)
            {
                ...
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            <li>
                                <button="type" class="btn btn-link view"><i class="fa-light fa-eye"></i> '.trans('file.View').'</button>
                            </li>';

                if(in_array("products-edit", $request['all_permission']))
                    $nestedData['options'] .= '<li>
                            <a href="'.route('products.edit', $product->id).'" class="btn btn-link"><i class="fa-light fa-pen-to-square"></i> '.trans('file.edit').'</a>
                        </li>';
                if(in_array("product_history", $request['all_permission']))
                    $nestedData['options'] .= \Form::open(["route" => "products.history", "method" => "GET"] ).'
                            <li>
                                ...
                                <button type="submit" class="btn btn-link"><i class="fa-light fa-list-timeline"></i> '.trans("file.Product History").'</button>
                            </li>'.\Form::close();
                if(in_array("print_barcode", $request['all_permission'])) {
                    ...
                    $nestedData['options'] .= \Form::open(["route" => "product.printBarcode", "method" => "GET"] ).'
                        <li>
                            ...
                            <button type="submit" class="btn btn-link"><i class="fa-light fa-barcode"></i> '.trans("file.print_barcode").'</button>
                        </li>'.\Form::close();
                }
                if(in_array("products-delete", $request['all_permission']))
                    $nestedData['options'] .= \Form::open(["route" => ["products.destroy", $product->id], "method" => "DELETE"] ).'
                            <li>
                              <button type="submit" class="btn btn-link" onclick="return confirmDelete()"><i class="fa-light fa-trash-can"></i> '.trans("file.delete").'</button>
                            </li>'.\Form::close().'
                        </ul>
                    </div>';
    ...
    {
        $data['reference_no'] = 'acom-' . date("Ymd") . '-'. date("his");
        ...
        Payment::create([
            'payment_reference' => 'pg-' . date("Ymd") . '-'. date("his"),
            'user_id' => Auth::id(),
            'purchase_id' => $purchase_data->id,
            'account_id' => 0,
