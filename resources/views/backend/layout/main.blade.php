...
<head>
  ...
  <!-- Font Awesome CSS-->
  <link rel="preload" href="<?php echo asset('fonts/fontawesome/free/css/all.css') ?>" as="style" onload="this.onload=null;this.rel='stylesheet'">
  <link rel="preload" href="<?php echo asset('fonts/fontawesome/pro/css/all.css') ?>" as="style" onload="this.onload=null;this.rel='stylesheet'">
  ...
  <link rel="stylesheet" href="<?php echo asset('css/propio-adentro.css') ?>" id="theme-stylesheet" type="text/css">
...
<link href="https://fonts.googleapis.com/css2?family=Nunito+Sans:opsz,wght@6..12,200..1000&family=Poppins:wght@100;200;300;400;500;600;700;800;900&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/sweetalert2@11.11.1/dist/sweetalert2.min.css" type="text/css">
  @stack('css')
</head>
...
<body class="@if($theme == 'dark')dark-mode dripicons-brightness-low @endif  @if(Route::current()->getName() == 'sale.pos') pos-page @endif" onload="myFunction()">
  ...
  <nav class="side-navbar">
    <span class="brand-big">
      ...
      <a href="{{url('/')}}"><img src="{{url('logo', $general_setting->site_logo)}}" width="160"></a>
      @else
      <a href="{{url('/')}}">
  ...
  <div class="page">
    ...
    <header class="container-fluid">
      <nav class="navbar">
        <a id="toggle-btn" href="#" class="menu-btn-mio"><i class="fa-regular fa-bars icono-top"></i></a>

        <ul class="nav-menu list-unstyled d-flex flex-md-row align-items-md-center">
          <div class="dropdown">
            
            <a class="nav-item" type="button" data-toggle="dropdown" aria-expanded="false">
              <i class="fa-regular fa-caret-down icono-top"></i>
            </a>
          ...
          ?>
          @if($sale_add_permission_active)
          <li class="nav-item"><a href="{{route('sale.pos')}}"><i class="fa-regular fa-cash-register icono-top"></i></a></li>
          @endif
          @if(config('database.connections.saleprosaas_landlord'))
          ...
          @endif
          <li class="nav-item"><a id="btnFullscreen" data-toggle="tooltip" title="{{trans('file.Full Screen')}}"><i class="fa-regular fa-expand-wide icono-top"></i></a></li>
          @if(\Auth::user()->role_id <= 2) <li class="nav-item"><a href="{{route('cashRegister.index')}}" data-toggle="tooltip" title="{{trans('file.Cash Register List')}}"><i class="fa-regular fa-shelves icono-top"></i></a></li>
            @endif
            ...
            <li class="nav-item" id="notification-icon">
              <a rel="nofollow" data-toggle="tooltip" title="{{__('Notifications')}}" class="nav-link dropdown-item"><i class="fa-regular fa-bell icono-top"></i><span class="badge badge-danger notification-number">{{$alert_product + $dso_alert_product_no + \Auth::user()->unreadNotifications->where('data.reminder_date', date('Y-m-d'))->count()}}</span>
              </a>
            ...
            <li class="nav-item" id="notification-icon">
              <a rel="nofollow" data-toggle="tooltip" title="{{__('Notifications')}}" class="nav-link dropdown-item"><i class="fa-light fa-bell"></i><span class="badge badge-danger notification-number">{{\Auth::user()->unreadNotifications->where('data.reminder_date', date('Y-m-d'))->count()}}</span>
              </a>
              <ul class="right-sidebar">
                @foreach(\Auth::user()->unreadNotifications->where('data.reminder_date', date('Y-m-d')) as $key => $notification)
                <li class="notifications">
            ...
            @endif
            <li class="nav-item">
              <a rel="nofollow" title="{{trans('file.language')}}" data-toggle="tooltip" class="nav-link dropdown-item"><i class="fa-regular fa-globe icono-top"></i></a>
              <ul class="right-sidebar">
                <li>
                  <a href="{{ url('language_switch/en') }}" class="btn btn-link idioma-menu"><img src="https://tienda.distribuidoratorrefuerte.com/public/images/flags/united-states-of-america.png" height="25" width="30"> English</a>
                </li>
                <li>
                  <a href="{{ url('language_switch/es') }}" class="btn btn-link idioma-menu"><img src="https://tienda.distribuidoratorrefuerte.com/public/images/flags/guatemala.png" height="25" width="30"> Español</a>
                </li>
              </ul>
            </li>
            <li class="nav-item">
              <a rel="nofollow" data-toggle="tooltip" class="nav-link dropdown-item"><i class="fa-regular fa-user"></i> <span>{{ucfirst(Auth::user()->name)}}</span> <i class="fa-light fa-chevron-down"></i>
              </a>
              <ul class="right-sidebar">
                <li>
                  <a href="{{route('user.profile', ['id' => Auth::id()])}}"><i class="fa-light fa-user" style="
    margin-right: 10px !important;
"></i> {{trans('file.profile')}}</a>
                </li>
                @if($general_setting_permission_active)
                <li>
                  <a href="{{route('setting.general')}}"><i class="fa-light fa-sliders" style="
    margin-right: 10px !important;
"></i> {{trans('file.settings')}}</a>
                </li>
                @endif
                <li>
                  <a href="{{url('my-transactions/'.date('Y').'/'.date('m'))}}"><i class="fa-light fa-money-from-bracket" style="
    margin-right: 10px !important;
"></i> {{trans('file.My Transactions')}}</a>
                </li>
                @if(Auth::user()->role_id != 5)
                <li>
                  <a href="{{url('holidays/my-holiday/'.date('Y').'/'.date('m'))}}"><i class="fa-light fa-calendar-days" style="
    margin-right: 10px !important;
"></i> {{trans('file.My Holiday')}}</a>
                </li>
                @endif
                @if($empty_database_permission_active)
                <li>
                  <a onclick="return confirm('¿Está seguro que quieres vaciar la base de datos? Si hace esto, todos sus datos se perderán.')" href="{{route('setting.emptyDatabase')}}"><i class="fa-light fa-database" style="
    margin-right: 10px !important;
"></i> {{trans('file.Empty Database')}}</a>
                </li>
                @endif
                <li>
                  <a href="{{ route('logout') }}" onclick="event.preventDefault();
                                        document.getElementById('logout-form').submit();"><i class="fa-light fa-right-from-bracket" style="
    margin-right: 10px !important;
"></i>
                    {{trans('file.logout')}}
                  </a>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="row">
              ...
              <div class="col-md-6 form-group">
                ...
                <select name="parent_id" class="form-control selectpicker" id="parent">
                  <option value="">Sin {{trans('file.parent')}}</option>
            ...
            <div class="form-group">
              ...
              <button type="submit" class="btn btn-primary category-submit-btn"><i class="fa-regular fa-floppy-disk icon-boton-primary"></i>{{trans('file.submit')}}</button>
            </div>
          </div>
          {{ Form::close() }}
        </div>
  ...
  @endif
  @stack('scripts')
 <script type="text/javascript" src="https://cdn.jsdelivr.net/npm/sweetalert2@11.11.1/dist/sweetalert2.all.min.js"></script>
